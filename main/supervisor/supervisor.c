...
static bool is_timeout(task_health_t *task, TickType_t now)
{
    ...
    return elapsed >
        pdMS_TO_TICKS(task->config.timeout_ms);
}


static bool is_stuck(task_health_t *task)
{
    return task->progress_counter ==
           task->last_progress;
}
...
static bool recover_task(task_id_t id)
{
    switch(id)
    {
        ...
        case TASK_TEMP:
            ...
            ESP_LOGW(TAG,
                "Temperature recovery not implemented");
    ...
    supervisor_task_config_t *config)
{
    ...
    g_task_health[id].last_heartbeat =
        xTaskGetTickCount();

    memcpy(
        &g_task_health[id].config,
        config,
        sizeof(supervisor_task_config_t));
